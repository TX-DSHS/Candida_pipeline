...
def prep_SRA_submission(result, run_name):
    ...
    results = pd.read_csv("/bioinformatics/Candida_auris/mycosnp-nf/output/{}/{}".format(run_name,result), header=0, sep ='\t', index_col=False, dtype=str)
...
# Rename columns in dataframe and add columns for each sample fastq path and instrument name    
          results = results.rename({"WGS_ID":"sample_id","SourceSite":"sourceSite","Submitter":"submitter","IsolatDate":"collection_date"}, axis='columns')
